...
ser = serial.Serial(
    ...
    baudrate=9600,         # Configura el baud rate según tu dispositivo
    parity=serial.PARITY_NONE,
    stopbits=serial.STOPBITS_ONE,
    bytesize=serial.EIGHTBITS,
...
def query_device(device_id):
    ...
    if response:
        ...
        if len(bins) == 4:
            try:
                # Convierte a valores numéricos y muestra cada bin en pantalla
                bin_a = float(bins[0])
                bin_b = float(bins[1])
                bin_c = float(bins[2])
                bin_d = float(bins[3])
                
                print(f"Bin A: {bin_a}, Bin B: {bin_b}, Bin C: {bin_c}, Bin D: {bin_d}")
                
                # Aquí podrías agregar el código para guardar en la base de datos si los datos son correctos
            except ValueError:
                print("Error de formato en la respuesta:", response)
        else:
            print("Respuesta inesperada:", response)
...
try:
    while True:
        for device_id in device_ids:
            query_device(device_id)
            time.sleep(1)  # Espera un segundo entre consultas
        time.sleep(10)  # Espera 10 segundos antes del próximo ciclo
except KeyboardInterrupt:
    print("Proceso terminado por el usuario.")
